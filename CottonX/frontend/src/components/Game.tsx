...
import RecursiveChat, { MODELS_BY_PROVIDER as DEFAULT_MODELS } from './RecursiveChat';
...
const Game = ({ userId, walletAddress }: { userId: string, walletAddress: string }) => {
    ...
    const [llmModelId, setLlmModelId] = useState<string>(() => {
        if (typeof window !== 'undefined') {
            return localStorage.getItem('llmModelId') || 'gemini-1.5-flash';
        }
    ...
    const [modelsByProvider, setModelsByProvider] = useState<Record<string, { id: string, name: string }[]>>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('modelsByProvider');
    ...
    useEffect(() => {
        localStorage.setItem('llmModelId', llmModelId);
    }, [llmModelId]);

    useEffect(() => {
        localStorage.setItem('modelsByProvider', JSON.stringify(modelsByProvider));
    }, [modelsByProvider]);
        ...
        const initializeGame = async () => {
            ...
            if (!charactersRef.current) {
                ...
                godRef.current = new God(
                    ...
                    (error) => {
                        console.warn(`God error: ${JSON.stringify(error)}`);
            ...
            mapImage.src = '/cottonx_office_map.png';
    ...
    useEffect(() => {
        if (godRef.current) {
            ...
            godRef.current.setLLMSettings(llmProvider, llmModelId);
        }
    }, [chatMode, llmProvider, llmModelId]);
